...
(function($) {
	$.fn.barousel = function(callerSettings) {
		var settings = $.extend({
			...
			navType : 1, // 1: boxes navigation; 2: prev/next navigation; 3:
			// custom navigation
		...
		settings.imageList = settings.imageWrapper.find('img').not(
				'[class*=intro]'); // list of the items' background image
		// (intro image is ignored)
		settings.contentList = settings.contentWrapper.find('div').not(
				'[class*=intro]'); // list of the items' content (intro content
		// is ignored)
		settings.contentLinksList = settings.contentLinksWrapper.find('a'); // list
		// of
		// the
		// items'
		// content
		...
		settings.contentIntro = settings.contentWrapper
		...
		settings.introActive = 0; // flag to know if there is an intro state
		// and if it's active

		settings.navWrapper[0].innerHTML = "";
		...
		if (settings.totalItem == 1) {
			...
			if (settings.imageList.length != settings.contentList.length) {
				...
				if (settings.debug == 1)
					console
							.log('[Barousel error] images and contents must be the same number');
				return this;
			}
			...
			if (settings.contentResize == 1 && settings.introActive == 0) {
				$(settings.contentWrapper)
						.height(
								$(settings.contentList[settings.currentIndex])
										.height() + 10);
			}

			// init the content link default state (optional)
			if (settings.contentLinksWrapper != null) {
				$(settings.contentLinksList[settings.currentIndex]).addClass(
						'current');
			}
			...
			if (settings.navType == 1) {
				...
				settings.imageList.each(function(n) {
					...
					strNavList += "<li><a href='#' title='"
							+ $(settings.contentList[n]).find('p.header')
									.text() + "' class='" + currentClass
							+ "'>&nbsp;</a></li>";
				});
				strNavList += "</ul>";
				settings.navWrapper.append(strNavList);
				settings.navList = settings.navWrapper.find('a'); // list of
				// the
				// items'
				// nav link
				...
				if ($(settings.imageList[0]).elevateZoom != undefined) {
					$(settings.imageList[0]).attr('data-zoom-image',
							$(settings.imageList[0]).attr('src'));
					$(settings.imageList[0]).elevateZoom({
						zoomType : "lens",
						lensShape : "round",
						lensSize : 200
					});
				...
				strNavList += "</ul><div class='counter'><span class='counter_current'>1</span>/<span class='counter_total'>"
						+ settings.totalItem + "</span></div>";
				settings.navWrapper.append(strNavList);
				settings.navList = settings.navWrapper.find('a'); // list of
				// the
				// items'
									...
									.click(
											function() {
												if (settings.navFreeze == 0) {
													window
															.clearTimeout(settings.timerCarousel);
													settings.stopCarousel = 1;
													if (settings.currentIndex != n
															|| settings.introActive == 1) {
														loadItem(settings, n);
			...
			if (settings.manualCarousel == 0) {
				...
				settings.timerCarousel = window.setTimeout(loadItemCall,
	...
	var loadItem = function(settings, index) {
		...
		/* DEBUG */
		if (settings.debug == 1) {
			console.log('[Barousel loadItem] currentImage:'
					+ currentImage.attr('src'));
			console.log('[Barousel loadItem] nextImage:'
					+ nextImage.attr('src'));
		}
		...
		if ($(nextImage).elevateZoom != undefined) {
			nextImage.attr('data-zoom-image', nextImage.attr('src'));
			$(nextImage).elevateZoom({
				zoomType : "lens",
				lensShape : "round",
				lensSize : 200
			});

			currentImage.removeAttr('data-zoom-image');
		}
	...
	var loadModuleContent = function(settings, index) {
		...
		if (settings.contentResize == 1
				&& parseInt($(settings.contentWrapper).height()) != parseInt($(
						settings.contentList[index]).height() + 10)) {
